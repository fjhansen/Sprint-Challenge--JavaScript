...
*/
const Dino = function(name,diet,weight,length,period) {
  this.name = name;
  this.diet = diet;
  this.weight = weight;
  this.length = length;
...
const stegosaurus = new Dino('stegosaurus','herbivorus','2000kg','9m','Late Jurassic')
...
for (let i = 0; i < graduates.length; i++) {
  let uniUni = graduates[i].university;
  if (uniUni.includes("Uni")) {
    unisWithUni.push(uniUni);
  };
};
...
const lowCaseAnimalNames = zooAnimals.map((animalName)=>{
  animalName.animal_name = animalName.animal_name.toLowerCase();
    return animalName.animal_name
